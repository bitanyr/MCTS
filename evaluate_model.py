...
def extract_detailed_costs(model, placement_dict):
    ...
    # [FIXED] Create a margin with a market floor of $20 to prevent Singularity
    ...
    total_real = capex + market + curt + gas_opex
...
def evaluate_and_compare(model_path="trained_adn_net_ep200.pth"):
    ...
    if os.path.exists(model_path):
        ...
        print(f"    Warning: Could not find '{model_path}'. Using random AI.")
    ...
    print(f"   -  Losses Cost (Implicitly Paid via Grid Purchase): Base=${base['loss_cost']:,.0f} | Rand=${rand_ai['loss_cost']:,.0f} | AI=${ai['loss_cost']:,.0f}")
    
    print(f"\n3. TOTAL ACTUAL COST (No Double-Counting):")
